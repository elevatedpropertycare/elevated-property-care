      ...
      <section className="relative bg-coastal-950 text-white py-24 px-4 sm:px-6 lg:px-8 text-center overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto space-y-6">
          ...
          <div className="pt-4 flex items-center justify-center space-x-4">
            ...
            >
              <span
                className={`inline-block h-5 w-5 transform rounded-full bg-sand-400 transition ${
                  billingCycle === 'annual' ? 'translate-x-7' : 'translate-x-0'
                }`}
              />
            </button>
            <span className={`text-xs uppercase tracking-wider font-semibold flex items-center space-x-1.5 ${billingCycle === 'annual' ? 'text-sand-300' : 'text-slate-400'}`}>
              <span>Annual View</span>
              <span className="bg-emerald-500/20 text-emerald-300 text-[10px] px-2 py-0.5 rounded border border-emerald-500/30 font-bold">12-Month Plan</span>
            </span>
          </div>
        </div>
      </section>
